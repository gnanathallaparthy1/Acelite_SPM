...
//	private func typeCastingByteToString(testCommand: TestCommandExecution?) -> [Int] {
//		
...
//		
//		let stateOfChargePropsInput = StateOfChargePropsInput(stateOfCharge: self.stateOfCharge ?? 0.0, currentEnergy: 0)
//		//"Cell_Volt_\(self.vehicleInfo?.vin ?? "")"
//		//"Pack_Voltage_\(self.vehicleInfo?.vin ?? "")"
//		let submitBatteryDataFilesPropsInput = SubmitBatteryDataFilesPropsInput(locationCode: LocationCode.init(rawValue: "AAA"), odometer: self.odometer ?? 0, totalNumberOfCharges: nil, lifetimeCharge: nil, lifetimeDischarge: nil, packVoltageFilename: "Pack_Voltage_\(self.vehicleInfo?.vin ?? "")", packCurrentFilename: "Pack_Current_\(self.vehicleInfo?.vin ?? "")", cellVoltagesFilename: "Cell_Volt_\(self.vehicleInfo?.vin ?? "")", transactionId: self.transactionId ?? "", vehicleProfile: submitBatteryDataVehicleProfileInput)
//		
